...
% \subsubsection{Discrete Freeway Model}
% \label{sub:freeway-model}


% We use the from Section~\ref{sec:continous-and-discrete-traffic-model-for-ramp-metering}
% Some notational changes from Section~\ref{sec:continous-and-discrete-traffic-model-for-ramp-metering} are made for readability reasons. The resulting discrete model has $T$ time-steps, $N$ spatial cells, and $N$ onramps and offramps.

% The state of cell $i\in \left[1,N\right]$ at time $k\in\left[1,T\right]$ is given by $\rho\left[i,k\right]$, while the number of vehicles on the adjacent onramp is given by $l\left[i,k\right]$. We require the following additional variables (specific to time-step $k$):

		% \begin{itemize}
		% 	\item $\delta\left[i,k\right]$: Maximum flow of vehicles exiting link $i$.
		% 	\item $\sigma\left[i,k\right]$: Maximum flow of vehicles etnering link $i$.
		% 	\item $d\left[i,k\right]$: Maximum flow of vehicles exiting onramp $i$.
		% 	\item $r^{\max}$: Physical capacity of onramp $i$.
		% 	\item $f^{\text{in}}\left[i,k\right]$: Actual flow entering link $i$.
		% 	\item $f^{\text{out}}\left[i,k\right]$: Actual flow exiting  link $i$.
		% 	\item $r\left[i,k\right]$: Actual flow exiting onramp $i$.
		% 	\item $\beta\left[i,k\right]$: Fraction of total flow from link $i$ entering link $i+1$ as opposed to offramp $i$.
		% 	\item $p$: Fraction of mainline flow given priority over onramp flow when merging in congestion.
		% 	\item $D\left[i,k\right]$: Flow entering onramp $i$.
		% \end{itemize}
		...
		% d\left[i,k\right] & = \min\left(l\left[i,k\right] / \triangle t, r^{\max}\right) \label{eq:onramp-demand} \\
		...
		% \begin{figure}[htbp]
		% 	\centering
		% 	\includegraphics[width=0.35\textwidth]{previous-articles/smart-america/diagrams/fw-junction}
		% 	\caption{}
		% 	\label{fig:fw-junction}
		% \end{figure}

		

		% Figure~\ref{fig:fw-junction} depicts a freeway junction labeled with the freeway state and control variables introduced above.
